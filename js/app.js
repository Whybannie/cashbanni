...
// ЕДИНЫЙ проверенный список предметов (никаких «призраков»)
function validInv(){ return S.inv.filter(i=>i&&i.gid&&gift(i.gid)); }

let upFrom=null, upTo=null, pickerMode=null, curCase=null, spinning=false, upBusy=false;
const IMG = {};
...
const sfx={ click:()=>beep(600,.05,'triangle',.07), tick:()=>beep(800+Math.random()*400,.03,'square',.04),
  win:()=>{beep(523,.1);setTimeout(()=>beep(659,.1),100);setTimeout(()=>beep(784,.2),200);},
  lose:()=>beep(180,.35,'sawtooth',.1),
  legend:()=>[523,659,784,1046,1318,1568].forEach((f,i)=>setTimeout(()=>beep(f,.16,'triangle',.12),i*90)) };
...
function renderHeader(){
  ...
  const li=levelInfo(); $('lvlText').textContent=li.lvl+' ур.'; $('lvlFill').style.width=(li.prog*100)+'%';
...
function renderAll(){ renderHeader(); renderFeed(); renderSection(document.querySelector('.tab.active').dataset.tab); }
function gotoCases(){
  document.querySelectorAll('.tab').forEach(x=>x.classList.remove('active'));
  document.querySelectorAll('.section').forEach(x=>x.classList.remove('active'));
  document.querySelector('[data-tab="cases"]').classList.add('active');
  $('sec-cases').classList.add('active'); renderCases();
}
...
function resetRoulette(){
  ...
  $('multiResults').classList.remove('active');
...
function spinOnce(g,dur){
  return new Promise(res=>{
    ...
    const w=94, cw=$('rouletteBox').offsetWidth;
    const off=winIdx*w-cw/2+w/2+rnd(-28,28);
...
async function spin(n){
  ...
  for(let i=0;i<n;i++){
    ...
    if(g.rarity==='legendary'){ sfx.legend(); confetti(150); toast('👑 ЛЕГЕНДА: '+g.name+'!','good'); }
    else sfx.win();
...
function itemCard(g,noActs){
  return '<div class="inv-item">'+
    ...
    (noActs?'':'<div class="acts"><button class="mini-btn sell" onclick="sellItem(\''+g.uid+'\')">Продать ⭐'+Math.floor(g.price*ECO.SELL_BACK)+'</button>'+
...
function renderInventory(){
  ...
  $('invValue').textContent='Стоимость: ⭐'+fmt(val);
...
function sellItem(u){
  ...
  save(); renderHeader(); renderInventory(); checkAch();
}
function sellAll(){
  const items=validInv();
  ...
  save(); renderHeader(); renderInventory();
}
function toUpgrade(u){
  ...
  $('sec-upgrade').classList.add('active'); renderUpgrade();
}
...
function renderUpgrade(){
  ...
  if(!upFrom && items.length===0){
    $('upFrom').innerHTML='<div class="muted">Инвентарь пуст</div><button class="btn btn-secondary" style="margin-top:8px" onclick="gotoCases()">🎁 Открыть кейс</button>';
  } else {
    $('upFrom').innerHTML=upFrom?gImg(gift(upFrom.gid))+'<div>'+gift(upFrom.gid).name+'</div><div style="color:var(--gold);font-weight:800">⭐'+fmt(gift(upFrom.gid).price)+'</div>':'— выбери —';
  }
  $('upTo').innerHTML=upTo?gImg(upTo)+'<div>'+upTo.name+'</div><div style="color:var(--gold);font-weight:800">⭐'+fmt(upTo.price)+'</div>':'— выбери —';
...
function doUpgrade(){
  ...
  spinWheel($('upWheel'),[{pct:ch,color:'#22c55e'},{pct:100-ch,color:'#2a2a4a'}],win?0:1,()=>{
    ...
    save(); renderHeader(); renderUpgrade(); checkAch();
  });
}

function openPicker(mode){
  pickerMode=mode; sfx.click();
  if(mode==='to'){
    const min=upFrom?gift(upFrom.gid).price:0;
    $('pickerGrid').innerHTML=GIFTS.filter(g=>g.price>min).map(g=>
      '<div class="inv-item" onclick="pickItem(\''+g.id+'\')">'+gImg(g)+'<div class="name">'+g.name+'</div><div class="price">⭐'+fmt(g.price)+'</div></div>').join('');
    $('pickerTitle').textContent='Выбери цель';
  } else {
    const items=validInv();
    $('pickerGrid').innerHTML=items.map(i=>{const g=gift(i.gid);return
      '<div class="inv-item" onclick="pickItem(\''+i.uid+'\')">'+gImg(g)+'<div class="name">'+g.name+'</div><div class="price">⭐'+fmt(g.price)+'</div></div>';}).join('')
      ||'<div class="muted">Инвентарь пуст — открой кейс 🎁</div>';
    $('pickerTitle').textContent='Твой инвентарь ('+items.length+')';
  }
  $('pickerModal').classList.add('active');
}
function closePicker(){ $('pickerModal').classList.remove('active'); }
function pickItem(id){
  if(pickerMode==='from'){ upFrom=S.inv.find(i=>i.uid===id)||null; renderUpgrade(); }
  if(pickerMode==='to'){ upTo=gift(id)||null; renderUpgrade(); }
  closePicker();
}
...
function renderMarket(){
  ...
  if(note) note.innerHTML='Официальный магазин: покупка 100% · продажа 70% · <b style="color:var(--ok)">скидка дня −20%: '+gift(disc).name+'</b>';
...
function buyGift(id,p){
  ...
  save(); renderHeader(); renderMarket(); renderInventory(); checkAch();
}
...
function runBattle(host,bet){
  ...
  $('battlePlayers').innerHTML=
    '<div class="b-player" id="bp1"><span class="emoji">😎</span><b>Ты</b></div>'+
...
function renderTop(){
  ...
  rows.push({name:'ТЫ',won:S.stats.won,me:true}); rows.sort((a,b)=>b.won-a.won);
...
function renderProfile(){
  const st=S.stats;
  $('statsGrid').innerHTML=[['Открыто кейсов',st.opened],['Потрачено','⭐'+fmt(st.spent)],['Выиграно','⭐'+fmt(st.won)],
    ['Лучший дроп','⭐'+fmt(st.best)],['Апгрейдов',st.upWins+'/'+st.upgrades],['Баттлов',st.bWins+'/'+st.battles],
    ['Продаж',st.sells],['Покупок',st.buys],['Сборка','build '+BUILD],['Предметов',validInv().length]].map(x=>'<div class="stat-card"><b>'+x[1]+'</b><span>'+x[0]+'</span></div>').join('');
...
// АВТО-ОЧИСТКА «призрачных» предметов + старт
